...
import cn.westward.estore.dao.UserDao;
...
import cn.westward.estore.utils.MailUtils;
...
public class UserServiceImpl implements UserService{
	...
	public void regist(User user) throws RegistException {
		...
		try {
			new UserDao().addUser(user);//将user bean存入数据库

			// 发送邮件操作
			String emailMsg = "注册成功，请在12小时内<a href='http://localhost:8090/bookEstore/user?method=activeuser&activecode="
					+ user.getActivecode()
					+ "'>激活</a>，激活码是"
					+ user.getActivecode();
